...
export const teams = {
    ...
    try{
        const info = await pool.query(`SELECT * FROM teams;`);
        res.json(info.rows)
    }
    catch(err){
        console.log(err)
    ...
    post : async function(req, res){
        try{
            ...
            const info = await pool.query(`INSERT INTO teams(team_name, club_id, group_id, coach_name)
    ...
    delete : async function(req, res){
        try{
            const {id} = req.params
            const info = await pool.query(`DELETE FROM teams where id=$1 RETURNING *`, [id])
